...
public class GWMBMSRecord implements BerType, Serializable {
	...
	private CTEID commonTeid = null;
	private PDPAddress iPMulticastSourceAddress = null;
	
	public GWMBMSRecord() {
	}

	public GWMBMSRecord(byte[] code) {
		this.code = code;
	}
	...
	public void setCommonTeid(CTEID commonTeid) {
		this.commonTeid = commonTeid;
	}

	public CTEID getCommonTeid() {
		return commonTeid;
	}

	public void setIPMulticastSourceAddress(PDPAddress iPMulticastSourceAddress) {
		this.iPMulticastSourceAddress = iPMulticastSourceAddress;
	}

	public PDPAddress getIPMulticastSourceAddress() {
		return iPMulticastSourceAddress;
	}
	...
	public int encode(OutputStream reverseOS, boolean withTag) throws IOException {
		...
		if (iPMulticastSourceAddress != null) {
			sublength = iPMulticastSourceAddress.encode(reverseOS);
		...
		if (commonTeid != null) {
			codeLength += commonTeid.encode(reverseOS, false);
	...
	public int decode(InputStream is, boolean withTag) throws IOException {
		...
		if (totalLength == -1) {
			...
			if (berTag.equals(BerTag.CONTEXT_CLASS, BerTag.PRIMITIVE, 17)) {
				commonTeid = new CTEID();
				subCodeLength += commonTeid.decode(is, false);
				subCodeLength += berTag.decode(is);
			}
			if (berTag.tagNumber == 0 && berTag.tagClass == 0 && berTag.primitive == 0) {
				int nextByte = is.read();
			...
			if (berTag.equals(BerTag.CONTEXT_CLASS, BerTag.CONSTRUCTED, 18)) {
				subCodeLength += length.decode(is);
				iPMulticastSourceAddress = new PDPAddress();
				int choiceDecodeLength = iPMulticastSourceAddress.decode(is, null);
				if (choiceDecodeLength != 0) {
					subCodeLength += choiceDecodeLength;
					subCodeLength += berTag.decode(is);
				}
				else {
					iPMulticastSourceAddress = null;
				}
		...
		while (subCodeLength < totalLength) {
			...
			if (berTag.equals(BerTag.CONTEXT_CLASS, BerTag.PRIMITIVE, 0)) {
				...
				subCodeLength += new BerLength().decode(is);
				iPMulticastSourceAddress = new PDPAddress();
				subCodeLength += iPMulticastSourceAddress.decode(is, null);
			}
		}
		if (subCodeLength != totalLength) {
	...
	public void appendAsString(StringBuilder sb, int indentLevel) {
		...
		if (commonTeid != null) {
			...
			sb.append("commonTeid: ").append(commonTeid);
		}
		...
		if (iPMulticastSourceAddress != null) {
			...
			sb.append("iPMulticastSourceAddress: ");
			iPMulticastSourceAddress.appendAsString(sb, indentLevel + 1);
		}
